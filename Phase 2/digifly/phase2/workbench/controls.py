...
CONTROL_SPECS: List[ControlSpec] = [
    ...
    ControlSpec(
        key="live_cache_enabled",
        label="Use live simulation cache",
        section="Runtime",
        control_type="bool",
        default=False,
        help_text="Shared runner only. Keeps a built NEURON network alive for repeated runtime-safe reruns.",
        runner_scopes=("shared_runner",),
        cache_impact="analysis_only",
    ),
    ControlSpec(
        key="live_cache_nproc",
        label="Live cache MPI processes",
        section="Runtime",
        control_type="int",
        default=1,
        help_text="Shared runner only. Values above 1 launch the live cache under mpiexec.",
        runner_scopes=("shared_runner",),
        cache_impact="analysis_only",
    ),
    ControlSpec(
        key="live_cache_force_restart",
        label="Restart live cache",
        section="Runtime",
        control_type="bool",
        default=False,
        help_text="Shared runner only. Stop and rebuild the live cache before the next run.",
        runner_scopes=("shared_runner",),
        cache_impact="analysis_only",
    ),
    ControlSpec(
        key="live_cache_session_root",
        label="Live cache session root",
        section="Runtime",
        control_type="text",
        default="",
        help_text="Shared runner only. Leave blank to use Phase 2/workbench_runs/_phase2_live_cache/<run_id>.",
        runner_scopes=("shared_runner",),
        cache_impact="analysis_only",
    ),
    ControlSpec(
        key="live_cache_auto_install_python",
        label="Auto-install Python packages",
        section="Runtime",
        control_type="bool",
        default=True,
        help_text="Shared runner only. Lets notebooks pip-install missing Python packages into the active kernel.",
        runner_scopes=("shared_runner",),
        cache_impact="analysis_only",
    ),
    ControlSpec(
