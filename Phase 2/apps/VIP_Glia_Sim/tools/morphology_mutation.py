...
_SWC_HINT_CACHE_BY_ROOT: Dict[str, Dict[int, List[Path]]] = {}
...
@dataclass
class MutationAISPolicy:
    neuron_id: int
    primary_node_id: Optional[int]
    primary_xloc: float
    extra_node_ids: List[int]
    note: str = ""
...
def _swc_hint_maps(swc_root: Path) -> Dict[int, List[Path]]:
    key = str(swc_root.resolve())
    cached = _SWC_HINT_CACHE_BY_ROOT.get(key)
    if cached is not None:
        return cached

    out: Dict[int, List[Path]] = {}
    candidates = [
        swc_root / "_ais_cache.csv",
        swc_root.parent / "notebooks" / "_ais_visualizer_outputs" / "plots" / "_ais_cache_interactive.csv",
    ]
            ...
            with csv_path.open("r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        continue
                    raw = str(row[6]).strip().strip('"').strip("'")
                    if not raw:
                        continue
                    p = Path(raw).expanduser()
                    if not p.is_absolute():
                        p = (swc_root / p).resolve()
                    try:
                        p = p.resolve()
                    except Exception:
                        continue
                    if not p.exists():
                        continue
                    if swc_root not in p.parents and p.parent != swc_root:
                        continue
                    out.setdefault(nid, []).append(p)
        except Exception:
            continue
    _SWC_HINT_CACHE_BY_ROOT[key] = out
    return out


def _score_swc_candidate(p: Path, nid: int) -> tuple:
    name = p.name.lower()
    return (
        0 if "axodendro_with_synapses" in name else 1,
        0 if "healed_final" in name else 1,
        0 if "healed" in name else 1,
        len(str(p)),
        str(p),
    )


def _fast_find_swc_path(swc_root: Path, nid: int) -> Optional[Path]:
    hint_map = _swc_hint_maps(swc_root)
    hinted = [p for p in hint_map.get(int(nid), []) if p.exists()]
    if hinted:
        return min(hinted, key=lambda p: _score_swc_candidate(p, nid)).resolve()
    ...
    for pat in patterns:
        hits = sorted(swc_root.glob(pat), key=lambda p: _score_swc_candidate(p, nid))
        if hits:
...
def find_swc_path(swc_root: str | Path, neuron_id: int) -> Path:
    ...
    fast = _fast_find_swc_path(swc_root, nid)
    if fast is not None:
        return fast
...
class MorphologyMutationProject:
    def __init__(self, swc_root: str | Path, neuron_ids: Sequence[int]):
        ...
        self.ais_policies: Dict[int, Dict[str, Any]] = {
            int(nid): {
        ...
        self.recording_policies: Dict[int, Dict[str, set[int]]] = {
            int(nid): {"probe": set()} for nid in self.neuron_ids
        }
    ...
    def _ais_bucket(self, neuron_id: int) -> Dict[str, Any]:
        nid = int(neuron_id)
        if nid not in self.tables:
            raise KeyError(f"Neuron id not loaded: {nid}")
        if nid not in self.ais_policies:
            self.ais_policies[nid] = {
                "primary_node_id": None,
                "primary_xloc": 0.5,
                "extra_node_ids": set(),
            }
        bucket = self.ais_policies[nid]
        bucket["primary_xloc"] = float(np.clip(float(bucket.get("primary_xloc", 0.5)), 0.0, 1.0))
        if not isinstance(bucket.get("extra_node_ids"), set):
            bucket["extra_node_ids"] = set(int(x) for x in (bucket.get("extra_node_ids") or []))
        return bucket

    def _recording_bucket(self, neuron_id: int) -> Dict[str, set[int]]:
        nid = int(neuron_id)
        if nid not in self.tables:
            raise KeyError(f"Neuron id not loaded: {nid}")
        if nid not in self.recording_policies:
            self.recording_policies[nid] = {"probe": set()}
        bucket = self.recording_policies[nid]
        if not isinstance(bucket.get("probe"), set):
            bucket["probe"] = set(int(x) for x in (bucket.get("probe") or []))
        return bucket
    ...
    def set_nodes_recording(
        ...
        *,
        ...
        nid = int(neuron_id)
        target = self._expand_node_targets(nid, node_ids, include_subtree=bool(include_subtree))
        bucket = self._recording_bucket(nid)
        if replace:
            bucket["probe"] = set(int(x) for x in target)
        else:
            bucket["probe"].update(int(x) for x in target)

        changed = sorted(int(x) for x in bucket["probe"])
        self._record_op(
            nid,
            "set_nodes_recording",
            {
    ...
    def recording_policy(self, neuron_id: int) -> Dict[str, Any]:
        nid = int(neuron_id)
        bucket = self._recording_bucket(nid)
        return {
            "neuron_id": int(nid),
            "probe_node_ids": sorted(int(x) for x in bucket.get("probe", set())),
        }

    def _recording_policies_manifest(self) -> Dict[str, Dict[str, Any]]:
        out: Dict[str, Dict[str, Any]] = {}
        for nid in sorted(self.tables.keys()):
            pol = self.recording_policy(int(nid))
            out[str(int(nid))] = {
                "probe_node_ids": [int(x) for x in pol.get("probe_node_ids", [])],
            }
        return out
    ...
    def set_ais_nodes(
        ...
        *,
        ...
        primary_xloc: float = 0.5,
        replace: bool = False,
        note: str = "",
    ) -> Dict[str, Any]:
        nid = int(neuron_id)
        target = self._expand_node_targets(nid, node_ids, include_subtree=False)
        if not target:
            raise ValueError("node_ids must contain at least one node")

        bucket = self._ais_bucket(nid)
        px = float(np.clip(float(primary_xloc), 0.0, 1.0))
        if primary_node_id is None:
            chosen_primary = int(target[-1])
        else:
            chosen_primary = int(primary_node_id)
            _require_nodes_exist(self.table(nid), [chosen_primary])
            if chosen_primary not in set(int(x) for x in target):
                target = sorted(set(int(x) for x in target).union({int(chosen_primary)}))

        extra_ids = sorted(int(x) for x in target if int(x) != int(chosen_primary))
        if replace:
            bucket["primary_node_id"] = int(chosen_primary)
            bucket["primary_xloc"] = float(px)
            bucket["extra_node_ids"] = set(int(x) for x in extra_ids)
        else:
            bucket["primary_node_id"] = int(chosen_primary)
            bucket["primary_xloc"] = float(px)
            bucket["extra_node_ids"].update(int(x) for x in extra_ids)
            bucket["extra_node_ids"].discard(int(chosen_primary))

        resolved_extra = sorted(int(x) for x in bucket["extra_node_ids"])
        changed = sorted(set([int(chosen_primary)]).union(int(x) for x in resolved_extra))
        self._record_op(
            nid,
            "set_ais_nodes",
            {
                "node_ids": [int(x) for x in node_ids],
                "resolved_node_ids": [int(x) for x in target],
                "primary_node_id": int(chosen_primary),
                "primary_xloc": float(px),
                "replace": bool(replace),
                "note": str(note),
                "resolved_extra_node_ids": resolved_extra,
            },
            changed,
        )
        return {
            "primary_node_id": int(chosen_primary),
            "primary_xloc": float(px),
            "extra_node_ids": resolved_extra,
        }

    def clear_ais_policy(
        self,
        neuron_id: int,
        *,
        clear_primary: bool = True,
        clear_extra: bool = True,
        note: str = "",
    ) -> List[int]:
        nid = int(neuron_id)
        bucket = self._ais_bucket(nid)
        changed: set[int] = set()
        if clear_primary and bucket.get("primary_node_id") is not None:
            changed.add(int(bucket["primary_node_id"]))
            bucket["primary_node_id"] = None
        if clear_extra:
            changed.update(int(x) for x in bucket.get("extra_node_ids", set()))
            bucket["extra_node_ids"] = set()
        self._record_op(
            nid,
            "clear_ais_policy",
            {
                "clear_primary": bool(clear_primary),
                "clear_extra": bool(clear_extra),
                "note": str(note),
            },
            sorted(changed),
        )
    ...
    def ais_policy(self, neuron_id: int) -> MutationAISPolicy:
        nid = int(neuron_id)
        bucket = self._ais_bucket(nid)
        primary = bucket.get("primary_node_id")
        return MutationAISPolicy(
            neuron_id=int(nid),
            primary_node_id=(None if primary is None else int(primary)),
            primary_xloc=float(bucket.get("primary_xloc", 0.5)),
            extra_node_ids=sorted(int(x) for x in bucket.get("extra_node_ids", set())),
        )

    def _ais_policies_manifest(self) -> Dict[str, Dict[str, Any]]:
        out: Dict[str, Dict[str, Any]] = {}
        for nid in sorted(self.tables.keys()):
            pol = self.ais_policy(int(nid))
            out[str(int(nid))] = {
                "primary_node_id": (None if pol.primary_node_id is None else int(pol.primary_node_id)),
                "primary_xloc": float(pol.primary_xloc),
                "extra_node_ids": [int(x) for x in pol.extra_node_ids],
            }
        return out
    ...
    def _write_ais_overrides_csv(
        self,
        *,
        out_csv: str | Path,
        mutated_paths: Optional[Dict[int, Path]] = None,
        source: str = "morphology_mutation",
    ) -> Optional[Path]:
        rows: List[List[Any]] = []
        for nid in self.neuron_ids:
            pol = self.ais_policy(int(nid))
            if pol.primary_node_id is None:
                continue
            df = self.table(int(nid))
            row = df.loc[df["id"] == int(pol.primary_node_id)]
            if row.empty:
                continue
            rr = row.iloc[0]
            swc_path = None
            swc_mtime = None
            if mutated_paths and int(nid) in mutated_paths:
                swc_path = str(Path(mutated_paths[int(nid)]).expanduser().resolve())
                try:
                    swc_mtime = float(Path(swc_path).stat().st_mtime)
                except Exception:
                    swc_mtime = None
            rows.append(
                [
                    int(nid),
                    int(pol.primary_node_id),
                    float(pol.primary_xloc),
                    float(rr["x"]),
                    float(rr["y"]),
                    float(rr["z"]),
                    str(source),
                    ("" if swc_path is None else swc_path),
                    ("" if swc_mtime is None else swc_mtime),
                    _utc_now(),
                ]
            )
        ...
        p = Path(out_csv).expanduser().resolve()
    ...
    def export_bundle(
        ...
        *,
        ...
        if write_phase2_overlay:
            ...
            overlay_ais_csv = self._write_ais_overrides_csv(
                out_csv=Path(overlay_dir) / "ais_overrides.csv",
                mutated_paths=mutated_paths,
            )
        ...
        manifest = {
            ...
            "ais_policies": self._ais_policies_manifest(),
            "recording_policies": self._recording_policies_manifest(),
            "validation": {str(k): v for k, v in validation.items()},
            "phase2_ais_overrides_csv": (str(overlay_ais_csv) if overlay_ais_csv is not None else None),
        }
        ...
        ais_json = run_dir / "mutation_ais_policies.json"
        ais_json.write_text(
            json.dumps(self._ais_policies_manifest(), indent=2),
            encoding="utf-8",
        )

        recording_json = run_dir / "mutation_recording_policies.json"
        recording_json.write_text(
            json.dumps(self._recording_policies_manifest(), indent=2),
            encoding="utf-8",
        )

        run_dir_ais_csv = self._write_ais_overrides_csv(
            out_csv=run_dir / "ais_overrides.csv",
            mutated_paths=mutated_paths,
        )
        ...
        return {
            ...
            "ais_policies_json": str(ais_json),
            "recording_policies_json": str(recording_json),
            "validation_json": str(val_json),
            "mutated_swcs": {int(k): str(v) for k, v in mutated_paths.items()},
            "phase2_overlay_dir": str(overlay_dir) if overlay_dir is not None else None,
            "phase2_ais_overrides_csv": (str(overlay_ais_csv) if overlay_ais_csv is not None else None),
            "ais_overrides_csv": (str(run_dir_ais_csv) if run_dir_ais_csv is not None else None),
        }
