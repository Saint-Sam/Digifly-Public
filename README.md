...
Public-facing Digifly project scaffold extracted from the current working source of truth: `Digifly-MASTER_MULTIPROCESS`.
...
- `Phase 1/filter_ids_by_size_and_export_swc.py`: Option 26 helper used by the Phase 1 notebook for Excel ID filtering and SWC export.
...
- Original Digifly working folders were not modified.
- Notebook outputs and execution counts were cleared for GitHub.
- Hardcoded neuPrint JWT strings were replaced with `NEUPRINT_TOKEN` lookups.
- Large generated data, SWC exports, notebook checkpoints, and cache/build artifacts are ignored.
